...
/*if(privilage=='a')
{
 printf("only user can provide feedback");
}*/
{
...
{
//
//printf("%s%s%s%s",detail.name,detail.pid,detail.age,detail.sex);
fprintf(source,"%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",detail.pid,detail.name,detail.address,detail.bloodgroup,detail.diagnosis,detail.sex,detail.test,detail.age);
...
{
printf("\n%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", detail.pid,detail.name,detail.address,detail.bloodgroup,detail.diagnosis,detail.sex,detail.test,detail.age);
...
printf("\n Company :- sjb infosystems\t\t \t Programmer :- Aishu");
printf("\n Front End :- 'c' \t\t \t Back End :- MS word,files");
printf("\n Contact Address:-SJB  COLLEGE OF ENGINEERING AND TECHNOLOGY\n \t\t ER.AISHWARYA \n\t\t 201301");
